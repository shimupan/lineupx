...
const ValorantAgents: React.FC = () => {
  ...
  return (
      ...
      <Layout>
        ...
        <div className="mt-auto pr-8"> {/* Added right padding to shift content */}
          <div className="flex flex-col p-4 rounded-lg">
            {/* Ability Buttons and Description */}
            <div className="flex justify-end items-start mb-4"> {/* Changed justify-between to justify-end */}
              {/* Ability Buttons */}
              <div className="flex space-x-4"> {/* Increased space between buttons */}
                {allAgents?.data
                  .filter((agent: Agent) => agent.displayName === agentDetails.selectedAgentName)
                  .map((agent: Agent) => (
                    agent.abilities
                      .filter((ability: Ability) => ability.slot !== 'Passive')
              ...
              {/* Ability Description */}
              <div className="w-1/2 ml-8"> {/* Increased left margin */}
                {selectedAbility ? (
                  <>
                    <h3 className="text-white font-bold mb-2 text-2xl">{selectedAbility.displayName}</h3>
                    <p className="text-gray-300 text-sm">{selectedAbility.description}</p>
                  </>
                ) : (
                  <p className="text-gray-400 text-sm">Select an ability to see its description</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center">
            ...
            >
              ...
              <span>I N</span>
              <div id="clip" className="absolute top-0 overflow-hidden w-full h-full border-[5px] border-double border-[#2761c3] shadow-inner shadow-[#195480]"></div>
            </button>
          </div>
        </div>
      </Layout>
