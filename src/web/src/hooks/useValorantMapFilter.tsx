...
export default function useValorantMapFilter(maps: ValorantMaps['data'] | undefined) {
  const filteredMaps = useMemo(() => {
    if (!maps) return [];
    
    return maps.filter(
      (map) => !EXCLUDED_MAPS.includes(map.displayName as typeof EXCLUDED_MAPS[number])
    );
  }, [maps]);
