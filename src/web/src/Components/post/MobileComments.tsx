...
const MobileComments: React.FC<MobileCommentsProps> = ({ 
  ...
  const [optionsBarPosition, setOptionsBarPosition] = useState({ top: 0, left: 0 });
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (showOptions && !threeDotsRef.current?.contains(event.target as Node)) {
  ...
  const handleOptionsClick = (event: React.MouseEvent<SVGElement>, comment: Comment) => {
        ...
        {latestComment && (
          <div className="mt-2">
            <div className="flex items-start space-x-2">
              <img
                src={latestComment.ProfilePicture || '/api/placeholder/32/32'}
                alt="avatar"
                className="w-8 h-8 rounded-full"
              />
              <div className="flex-1">
                <div className="flex items-center space-x-2">
                  <p className="text-sm font-medium">{latestComment.username}</p>
                  <p className="text-xs text-gray-400">
                    {timeAgo(new Date(latestComment.createdAt))}
                  </p>
                </div>
                <p className="text-sm text-gray-300 line-clamp-2">{latestComment.text}</p>
              </div>
            </div>
          </div>
        )}
              ...
              <div className="p-3 space-y-3">
                {comments.map((comment, index) => (
                  <div key={index} className="flex items-start space-x-2">
                    <img
                      src={comment.ProfilePicture || '/api/placeholder/32/32'}
                      alt="avatar"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between">
                        <div className="flex flex-col flex-grow min-w-0">
                          ...
                          {isEditing && activeComment?._id === comment._id ? (
                        ...
                        {(Auth?._id === comment.user || Auth?.role === 'admin') && (
                          <div ref={threeDotsRef} className="ml-2 flex-shrink-0">
                            <BsThreeDotsVertical
                              size={16}
                              className="cursor-pointer text-gray-400 hover:text-gray-200"
                              onClick={(e) => handleOptionsClick(e, comment)}
                            />
                          </div>
                        )}
                      </div>
